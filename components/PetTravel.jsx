...
const AIRLINES = [
  ...
  },
  {
    ...
    scope: "domestic-only",
    tags: ["us"],
    cabin: "Yes — US domestic only",
    cabinStatus: "yes",
    direction: "Cabin allowed: domestic US only. Cabin NOT allowed: all international routes — Frontier stopped accepting pets (other than service animals) on international flights, so Mexico and Caribbean cabin pet travel is no longer available.",
    originAllowed: { uk: "no", us: "yes", eu: "no", india: "no", canada: "no", uae: "no", caribbean: "no", mexico: "no", "south-america": "no", "central-america": "no", japan: "no", korea: "no" },
  ...
  },
  {
    ...
    notes: "Strong UK-out option AND excellent cabin route from India to USA via Zurich. Uniquely accommodating to brachycephalic (snub-nosed) breeds — allowed in cabin (most airlines ban them from cargo only; SWISS keeps them in cabin). Use Heathrow not Gatwick. Reserve no later than 72 hours before departure — SWISS confirms availability and books via the Service Centre. For dogs entering the USA from SWISS (since 1 Aug 2024 per CDC): only ZRH → JFK, MIA, ATL, LAX, IAD, or PHL are accepted, and the dog must be at least 6 months old.",
    intl: "Yes (most routes)",
    verified: "May 2026",
  ...
  },
  {
    ...
    cabinStatus: "conditional",
    direction: "Cabin allowed: most international routes via Istanbul including OUT of UK (LHR/MAN → Istanbul), USA ↔ Istanbul, India ↔ Istanbul (Delhi, Mumbai, Bangalore, Hyderabad), and many Asian/African destinations. Economy class only — business class cabin pets banned from April 21, 2026 (new bookings). Cabin NOT allowed: INTO UK from Istanbul. NB: Turkish bans pets to UK destinations in any class — cabin AND cargo, including connections. So Turkish is an OUT-of-UK option only; you cannot use Turkish to fly a pet TO the UK. Cabin also NOT allowed into Dubai DXB (UAE law applies regardless of airline), some Middle East destinations.",
    originAllowed: { uk: "yes", us: "yes", eu: "yes", india: "yes", canada: "yes", uae: "no", caribbean: "no", mexico: "no", "south-america": "yes", "central-america": "no", japan: "yes", korea: "yes" },
    destinationAllowed: { uk: "no", us: "yes", eu: "yes", india: "yes", canada: "yes", uae: "no", caribbean: "no", mexico: "no", "south-america": "yes", "central-america": "no", japan: "yes", korea: "yes" },
    fee: "International from ~$70 starting, varies by route — use Turkish's pet fee calculator for an exact quote",
    weight: "Pet + carrier max 8 kg (17.6 lb)",
...
const WORKAROUND_ROUTES_TABLE = [
  ...
  // USA / Europe → Ireland (limited cabin: Iberia MAD→DUB, else ferry)
  {
    from: "USA or Europe",
    to: "Dublin / Ireland",
  ...
  return (
    ...
    >
      ...
      {/* Mobile menu — a normal in-flow dropdown. Capped to the viewport and
          ...
          Uses dvh (dynamic viewport height) so the cap is correct on mobile
          Safari, where the address bar makes 100vh taller than the visible
          area. Background is fully opaque so page content can't bleed through. */}
      {open && (
        <div
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {isSpecialFilter ? (
          ...
          /* Special explainer card — no airline carries cabin pets into the
             UK (a legal ban). Ireland is different: cabin pets CAN fly in,
             but options are very limited, so the workaround still matters. */
          <div className="bg-white border-2 border-amber-300 rounded-sm overflow-hidden">
            <div className="bg-stone-900 text-stone-50 px-6 py-5">
              ...
              <h3 className="font-serif text-2xl md:text-3xl leading-tight">
                No cabin pets into the UK — and very limited options into Ireland.
              </h3>
            </div>
